...
$('#classify-list li').on('click',function(){
  $('.click-list').removeClass('click-list').addClass('not-click');
  ...
  $('.classify-info').each(function(i){
    if (i==ClickValue){
      ...
      $(this).animate({"left":"5px"},"fast").css("display","block");

    }
    else {
      $(this).animate({"left":"-250px"},"fast").css("display","none");
...
function CreateAgeCharts(){
  new Chart(document.getElementById("age-chart"), {
    ...
    data: {
      ...
      datasets: [{
        label: "Age",
        backgroundColor: ["#478D96", "#1E6CC7","#1EBD21","#68B41D","#ABA91C","#A15A1B","#981B21"],
...
function CreateStoreyCharts(){
  new Chart(document.getElementById("storey-chart"), {
    ...
    data: {
      ...
      datasets: [{
        label: "Age",
        backgroundColor: ["#35BF36", "#33BC8F","#318BB9","#2F2FB6","#862DB3","#B02B84","#AE2929"],
...
map.on('click', function (e) {
      ...
      layers: ['Buildings Info']
        ...
        var PopupHTML = '<h3><strong>' + BuildingInfo[0].properties.name + '</strong></h3><p><strong>' +  BuildingInfo[0].properties.land_area +'</strong> meters tall </p> \
